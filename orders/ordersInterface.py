...
from utils.ui_components import create_combo_widget
...
class OrdersInterface(QWidget):
    ...
    def setup_ui(self):
        ...
        self.table_widget.setHorizontalHeaderLabels(
            ["", "单号", "类型", "货品id", "货品名称", "型号", "类别", "单价", "供应商", "归属项目", "状态", "经办人",
             "提交日期", "审核日期", "操作数量", "操作"])
    ...
    def get_count(self):
        ...
        if response.get("success") is True:
            print(f'count: {response.get("data")}')
    ...
    def load_data(self):
        try:
            ...
            # result = APIClient.get_request(url)
    ...
    def setup_table_row(self, row: int, order_info: dict):
        ...
        for col, key in enumerate(
                ['order_id', 'order_type', 'cargo_id', 'cargo_name', 'model', 'categories', 'price', 'provider',
                 'project', 'status', 'employee_name', 'published_at', 'processed_at', 'count']):
        ...
        # 为每行添加编辑按钮
        action_widget = create_combo_widget(
            callback=lambda: self.print_reciept_action(order_info['order_id'])
        )
        self.table_widget.setCellWidget(row, 15, action_widget)  # 将组件添加到每行的最后一列
    ...
    def adjust_column_widths(self):
        ...
        self.table_widget.setColumnWidth(1, 150)  # 第二列宽度固定为 200 像素
    ...
    def export_to_excel(self):
        category = self.categoriesComboBox.currentIndex()
        order_type = self.typeComboBox.currentIndex()
        order_status = self.statusComboBox.currentIndex()
        data = self.orders
        try:
            if category == 0 and order_type == 0 and order_status == 0:
                url = URL + '/orders/all'
                # response = APIClient.get_request(url)
                response = Worker.unpack_thread_queue(APIClient.get_request, url)
                if response.get("success") is True:
            ...
            if data:
                ...
                ws = wb.active
                ws.title = filename
                headers = ['单号', '类型', '货品id', '货品名称', '型号', '类别', '单价', '供应商', '项目', '状态',
                           '经办人', '提交日期', '审核日期', '操作数量']
                ws.append(headers)
    ...
    def import_from_excel(self):
        ...
        if not file_path:
            ...
            ws = wb.active
            expected_header = ['单号', '类型', '货品id', '货品名称', '型号', '类别', '单价', '供应商', '项目', '状态',
                           '经办人', '提交日期', '审核日期', '操作数量']
            ...
            if headers != expected_header:
                ...
                for row in ws.iter_rows(min_row=2, values_only=True):
                    order_id, order_type, cargo_id, cargo_name, model, categories, price, provider, project, status, employee_name, published_at, processed_at, count = row
                    if not all([order_id, order_type, cargo_id, cargo_name, model, categories, price, provider, project, status, employee_name, published_at, processed_at, count]):
                        skipped += 1
                    else:
                        info = {
                            'order_id': order_id,
